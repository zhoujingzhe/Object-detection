...
def generating_consequences(results):
    results = tf.convert_to_tensor(value=results, dtype='float32')
    Pc = results[:, :, :, 0]
    ...
    filtering_mask = K.greater_equal(Box_class_scores, 0.6)

    scores = tf.boolean_mask(Box_class_scores, filtering_mask)
    boxes = tf.boolean_mask(Boxes, filtering_mask)
    classes = tf.boolean_mask(Box_classes, filtering_mask)
    boxes = K.cast(boxes, 'float32')
    nms_indices = tf.image.non_max_suppression(boxes, scores, max_output_size = 5, iou_threshold=0.5)
    scores = K.gather(scores, nms_indices)
    boxes = K.gather(boxes, nms_indices)
    classes = K.gather(classes, nms_indices)
    scores = K.reshape(scores, (setting['batch_size'], -1, 1))
    boxes = K.reshape(boxes, (setting['batch_size'], -1, 4))
...
def Loss_v1(y_true, y_pred):
    ...
    ######
    Classification_loss = weight_Classification_loss * mat * (10 * K.cast(K.equal(C_index_pred, C_index_true), 'float32'))
