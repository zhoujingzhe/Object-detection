...
#   cannot use LRN local_response_normalization Why?
    ...
    X = Conv2D(filters=128, kernel_size=(2, 2), strides=2, padding='valid', kernel_initializer=glorot_uniform(seed=0))(
        X)  # 79x59
    X = BatchNormalization(axis=1)(X)
    X = LeakyReLU(alpha=0.3)(X)
    ...
    X = Conv2D(filters=256, kernel_size=(3, 3), strides=2, padding='valid', kernel_initializer=glorot_uniform(seed=0))(
        X)  # 39x29
    X = BatchNormalization(axis=1)(X)
    X = LeakyReLU(alpha=0.3)(X)
    ...
    X = AveragePooling2D(pool_size=(3, 3), strides=2, padding='valid')(X)  # 19x14
    
    X0 = Conv2D(filters=5, kernel_size=(1, 1), strides=1, padding='valid', kernel_initializer=glorot_uniform(seed=0))(
        X)  # 19x14
    X0 = Activation('sigmoid')(X0)
    
    X1 = Conv2D(filters=10, kernel_size=(1, 1), strides=1, padding='valid', kernel_initializer=glorot_uniform(seed=0))(
        X)  # 19x14
